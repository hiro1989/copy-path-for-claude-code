...
- [ ] 2.1 Write tests for config-gated sort integration in `copyPath` (sort applied when enabled, skipped when disabled, single-item no-op with sort enabled)
- [ ] 2.2 Add `copy-path-for-claude-code.sortPaths` boolean property (default `false`) to `package.json` contributes.configuration and integrate `sortPaths` into `copyPath` in `extension.ts`: read config, conditionally sort array before join for both explorer and editor paths
